...
/// </summary>
public sealed class FolderDS : CsvTreeDataSource<FolderM, FolderR, int> {
  public FolderDS(CoreR coreR, FolderR repo) : base(coreR.DB, "Folders", 3, repo) {
    IsDriveRelated = true;
  }

  protected override Dictionary<string, IEnumerable<FolderM>> _getAsDriveRelated() =>
    Repository.Tree.Category.Items.ToDictionary(x => x.Name, FolderR.GetAll<FolderM>);
